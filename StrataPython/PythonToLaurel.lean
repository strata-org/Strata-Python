...
/-- The wildcard modifies clause — one unguarded group whose only target is `*`,
    meaning the procedure may modify anything (`ModifiesGroup.wildcard`, which
    also documents why the single-group wrapping is load-bearing). -/
def wildcardModifies : List ModifiesGroup := ModifiesGroup.wildcard unknownSource
...
partial def translateDictKWords (ctx : TranslationContext) (kw : keyword SourceRange)
...
partial def pyKwordsToHashMap (kwords : List (keyword SourceRange)) : Std.HashMap String (expr SourceRange) :=
...
partial def isVarKwargs (kwords : List (keyword SourceRange)) : Bool :=
...
partial def translateVarKwargs (ctx : TranslationContext) (kwords : List (keyword SourceRange)) : Except TranslationError StmtExprMd := do
  match kwords[0]! with
  | .mk_keyword _ name expr =>
    match name.val with
...
partial def translateKwargs (ctx : TranslationContext) (kwords : List (keyword SourceRange)): Except TranslationError StmtExprMd := do
...
partial def removePosargsFromKwargs (kwords : List (keyword SourceRange)) (funcDecl: PythonFunctionDecl) : List (keyword SourceRange) :=
...
partial def combinePositionalAndKeywordArgs
    (posArgs: List (expr SourceRange))
    (kwords : List (keyword SourceRange))
    (funcDecl: Option PythonFunctionDecl)
    (displayName : String := "")
    (callRange : SourceRange)
      : Except TranslationError ((List (expr SourceRange)) × (List (keyword SourceRange)) × Bool):= do
  match funcDecl with
  | some funcDecl =>
...
partial def translateExprAsReceiver (ctx : TranslationContext)
    (e : expr SourceRange) : Except TranslationError StmtExprMd := do
  match e with
...
def translateExprExtractingCalls (ctx : TranslationContext) (e : expr SourceRange)
...
partial def getWithItemsVars (withItems: List (withitem SourceRange))
    :List (String × String) :=
...
partial def getForLoopVars (targetIter: expr SourceRange) :List (String × String) :=
...
def getExceptionCheckPreamble (ctx : TranslationContext) (e : StmtExprMd) (varName : String)
  ...
  -- Expressions embedding statement effects (e.g. module-global havoc) must
  -- be hoisted: duplicating them into the assert would re-run the effect and
  -- put a global write in a restricted position.
  let containsStatementEffect :=
    anyStmtExpr (fun node => match node.val with
      | .Assign _ _ => true
      | _ => false) e
  if (getMaybeExceptionExprs ctx e).isEmpty then
    ([], e)
  else if containsUserCall ctx e || containsStatementEffect then
...
/-- Whether the statements can transfer control out of an enclosing `try`
    statement: a `return` anywhere, or a `break`/`continue` that is not
    enclosed in a loop nested inside these statements. Nested function and
    class definitions are opaque scopes whose control flow cannot escape. -/
private partial def containsTryEscape (stmts : List (stmt SourceRange)) : Bool :=
  let rec go (inLoop : Bool) (s : stmt SourceRange) : Bool :=
    match s with
    | .Return _ _ => true
    | .Break _ | .Continue _ => !inLoop
    | .If _ _ thenb elseb =>
      thenb.val.toList.any (go inLoop) || elseb.val.toList.any (go inLoop)
    | .While _ _ wbody orelse =>
      wbody.val.toList.any (go true) || orelse.val.toList.any (go inLoop)
    | .For _ _ _ fbody orelse _
    | .AsyncFor _ _ _ fbody orelse _ =>
      fbody.val.toList.any (go true) || orelse.val.toList.any (go inLoop)
    | .With _ _ wbody _
    | .AsyncWith _ _ wbody _ => wbody.val.toList.any (go inLoop)
    | .Try _ tbody handlers orelse finalbody
    | .TryStar _ tbody handlers orelse finalbody =>
      tbody.val.toList.any (go inLoop) ||
        (handlers.val.toList.any fun h => match h with
          | .ExceptHandler _ _ _ hbody => hbody.val.toList.any (go inLoop)) ||
        orelse.val.toList.any (go inLoop) || finalbody.val.toList.any (go inLoop)
    | .Match _ _ cases =>
      cases.val.toList.any fun c => match c with
        | .mk_match_case _ _ _ cbody => cbody.val.toList.any (go inLoop)
    | _ => false
  stmts.any (go false)

mutual

partial def translateStmt (ctx : TranslationContext) (s : stmt SourceRange)
  ...
  | .While _ test body orelse => do
    ...
    -- A guard whose unmodeled calls havoc module globals may not appear in
    -- loop-condition position (restricted); evaluate it in statement position
    -- before the loop and at the end of every iteration.
    let writesScopedGlobal (e : StmtExprMd) : Bool :=
      anyStmtExpr (fun node =>
        match node.val with
        | .Assign targets _ => targets.any fun target =>
            match target.val with
            | .Local n => isScopedModuleGlobal ctx n.text
            | _ => false
        | _ => false) e
    if writesScopedGlobal condExpr then
      ...
      let exceptionChecks :=
        if (getMaybeExceptionExprs ctx condExpr).isEmpty then []
        else [mkExceptionCheckAssert condRef "Check exception"]
      let reEval := mkStmtExprMdWithLoc
        (StmtExpr.Assign [mkVariableMd (.Local condVar)] condExpr) md
      let continueBlock := mkStmtExprMdWithLoc
        (StmtExpr.Block bodyStmts (some continueLabel)) md
      let whileBody := mkStmtExprMd
        (StmtExpr.Block ([continueBlock, reEval] ++ exceptionChecks) none)
      let whileStmt := mkStmtExprMdWithLoc
        (StmtExpr.While (Any_to_bool condRef) [] none whileBody false) md
      let whileWrapped := mkStmtExprMdWithLoc
  ...
  | .Try _ body handlers orelse finalbody => do
    -- An early exit would jump past the finally body, which this lowering
    -- runs on fall-through only.
    if !finalbody.val.isEmpty &&
        (containsTryEscape body.val.toList ||
          handlers.val.toList.any (fun h => match h with
            | .ExceptHandler _ _ _ hbody => containsTryEscape hbody.val.toList) ||
          containsTryEscape orelse.val.toList) then
      throw (.unsupportedConstruct
        "try-finally with return, break, or continue in the try statement is not supported"
        (toString (repr s)))
    ...
    let markCompleted :=
      if orelse.val.isEmpty then []
        ...
        [mkVariableMd (.Local completedVar)]
        (mkStmtExprMd (.LiteralBool true)))]

    -- catchers block: body + completion mark + exit try (normal path)
    let catchersBlock := mkStmtExprMd (StmtExpr.Block
      (bodyStmtsWithChecks ++ markCompleted ++ [exitTry]) (some catchersLabel))
    ...
    let (completedDecl, elseCtx, elseStmts) ←
      if orelse.val.isEmpty then
        pure ([], bodyCtx, [])
      else do
        let decl := mkVarDeclInit completedVar (mkHighTypeMd .TBool)
          (mkStmtExprMd (.LiteralBool false))
        let (elseCtx, elseBody) ← translateStmtList bodyCtx orelse.val.toList
        let guardedElse := mkStmtExprMdWithLoc (StmtExpr.IfThenElse
          (mkStmtExprMd (StmtExpr.Var (.Local completedVar)))
          (mkStmtExprMd (StmtExpr.Block elseBody none)) none) md
        pure ([decl], elseCtx, [guardedElse])
    -- `finally` runs after the try on every fall-through path: normal
    -- completion, handled exceptions, and re-raised exceptions.
    let (finalCtx, finallyStmts) ← translateStmtList elseCtx finalbody.val.toList
    let mergedVars := finalCtx.variableTypes ++
      (handlerCtx.variableTypes.filter fun (n, _) =>
        !finalCtx.variableTypes.any fun (bn, _) => bn == n)
    let finalCtx := { finalCtx with variableTypes := mergedVars }
    return (finalCtx, completedDecl ++ [tryBlock] ++ elseStmts ++ finallyStmts)
  ...
  | .AugAssign sr target op value => do
    ...
    let rhs : expr SourceRange := .BinOp sr target op value
    let pyNormalAssign : stmt SourceRange :=
...
partial def translateStmtList (ctx : TranslationContext) (stmts : List (stmt SourceRange))
    : Except TranslationError (TranslationContext × List StmtExprMd) := do
  let mut currentCtx := ctx
...
partial def getNestedSubscripts (e :  expr SourceRange) : List (expr SourceRange) :=
  match e with
  | .Subscript _ val slice _ => [val] ++ (getNestedSubscripts slice)
  | _ => [e]

def getUnionTypes (arg: expr SourceRange) : List (expr SourceRange) :=
  match arg with
...
private def normalizeTypingName (e : expr SourceRange) : String :=
  match e with
...
partial def getArgumentTypes (arg: expr SourceRange) : Except TranslationError (List String) :=
  match arg with
...
def unpackPyArguments (ctx : TranslationContext) (args: arguments SourceRange)
...
def translateFunction (ctx : TranslationContext) (sourceRange: SourceRange) (funcDecl : PythonFunctionDecl)
    (body: Option (List (stmt SourceRange)))
...
def preludeSignatureToPythonFunctionDecl (prelude : Core.Program) : List PythonFunctionDecl :=
  prelude.decls.filterMap fun decl =>
    match Core.Program.Procedure.find? prelude decl.name with
    | some proc =>
      let noneexpr : expr SourceRange := .Constant default (.ConNone default) default
...
def extractClassFields (ctx : TranslationContext) (classBody : Array (stmt SourceRange))
...
def extractFieldsFromInit (ctx : TranslationContext) (initBody : Array (stmt SourceRange))
    : Except TranslationError (List Field) := do
  let mut fields : List Field := []
...
private def matchDispatchField (ctx : TranslationContext) (target value : expr SourceRange)
...
def translateClass (ctx : TranslationContext) (classStmt : stmt SourceRange)
...
private structure DirectImportBinding where
  localName : String
  moduleName : String
  importedName : Option String
  source : SourceRange

/-- Collect direct, absolute module imports with their local binding names. -/
private def collectDirectImportBindings
    (stmts : Array (stmt SourceRange)) : List DirectImportBinding :=
  stmts.toList.flatMap fun s => match s with
    | .Import sr aliases => aliases.val.toList.map fun a => match a with
      | .mk_alias _ moduleName asName =>
        let localName := asName.val.map (·.val) |>.getD (moduleName.val.splitOn ".").head!
        { localName, moduleName := moduleName.val, importedName := none, source := sr }
    | .ImportFrom sr moduleName aliases level =>
      let isAbsolute := level.val.all fun value => value.value == 0
      if !isAbsolute then [] else
        match moduleName.val with
        | none => []
        | some moduleName => aliases.val.toList.map fun a => match a with
          | .mk_alias _ importedName asName =>
            let localName := asName.val.map (·.val) |>.getD importedName.val
            { localName, moduleName := moduleName.val,
              importedName := some importedName.val, source := sr }
    | _ => []
...
private def collectModuleValueRebindings
    (stmts : Array (stmt SourceRange)) : Std.HashSet String :=
  let assigned := (collectModuleBoundNames stmts.toList).foldl
    (init := ({} : Std.HashSet String)) fun names (name, _) => names.insert name
  (collectModuleScopeDefinitions stmts.toList).foldl (init := assigned) fun names s =>
    match s with
    | .FunctionDef _ name _ _ _ _ _ _
    | .AsyncFunctionDef _ name _ _ _ _ _ _
    | .ClassDef _ name _ _ _ _ _ => names.insert name.val
    | _ => names

private def hasTrustedDirectImport
    (imports : List DirectImportBinding)
    (rebindings : Std.HashSet String)
    (localName moduleName : String)
    (importedName : Option String)
    (useSource : SourceRange) : Bool :=
  if rebindings.contains localName then false else
    let matching := imports.filter (·.localName == localName)
    !matching.isEmpty &&
      matching.all (fun binding =>
        binding.moduleName == moduleName && binding.importedName == importedName) &&
      matching.any (·.source.start.byteIdx <= useSource.start.byteIdx)
...
private partial def collectModuleGlobals (ctx : TranslationContext)
  ...
  let directImports := collectDirectImportBindings body
  let moduleValueRebindings := collectModuleValueRebindings body
  let isLoggingHandle (value : expr SourceRange) : Bool :=
    match value with
    | .Call _ (.Attribute _ (.Name _ moduleName _) methodName _) _ _ =>
      methodName.val == "getLogger" &&
        hasTrustedDirectImport directImports moduleValueRebindings
          moduleName.val "logging" none value.ann
    | _ => false
  ...
  let rec go (scopeGlobals : Option (Std.HashSet String))
    ...
    | .Assign _ targets value _ =>
      ...
      else if scopeGlobals.isNone && isLoggingHandle value then acc
    ...
    | .AnnAssign _ target annotation value _ =>
      ...
      | .Name sr n _ =>
        if !isActiveTarget scopeGlobals n.val then acc
          ...
          poison acc [(n.val, sr)]
        else if scopeGlobals.isNone && value.val.any isLoggingHandle then acc
...
def pythonToLaurel (info : PreludeInfo)
  ...
  let classesInHierarchy : Std.HashSet String :=
    ...
    | .ClassDef _ className bases _ _ _ _ =>
      let hasNontrivialBases := bases.val.toList.any fun b =>
        match b with
        | expr.Name _ ⟨_, "object"⟩ _ => false
        | _ => true
      let acc := bases.val.toList.foldl (init := acc) fun acc' b =>
        match b with
        | expr.Name _ ⟨_, baseName⟩ _ =>
  ...
  for stmt in moduleDefinitions do
    if let .ClassDef _ className _ _ classBody decoratorList _ := stmt then
      -- Accept dataclass only when the decorator name is bound directly to the
      -- standard-library module and has not been rebound in module scope.
      let isDataclassRef (d : expr SourceRange) : Bool :=
        match d with
        | .Name _ dName _ =>
          hasTrustedDirectImport directImports moduleValueRebindings
            dName.val "dataclasses" (some "dataclass") d.ann
        | .Attribute _ (.Name _ moduleName _) dName _ =>
          dName.val == "dataclass" &&
            hasTrustedDirectImport directImports moduleValueRebindings
              moduleName.val "dataclasses" none d.ann
        | _ => false
      let isKnownClassDecorator (d : expr SourceRange) : Bool :=
        match d with
        | .Call _ callee _ _ => isDataclassRef callee
        | _ => isDataclassRef d
      if decoratorList.val.toList.any (!isKnownClassDecorator ·) then
        throw (.unsupportedConstruct
  ...
  let mut otherStmts : Array (stmt SourceRange) := #[]
  ...
  let nameDecl : stmt SourceRange := .AnnAssign default (.Name default {val:= "__name__", ann:= default} default)
