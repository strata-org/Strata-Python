...
Ports `StrataPythonTest/Regex/diff_test.py` to a Lean test. The test corpus —
...
1. Runs the regex through Strata's SMT backend in-process (the same path
   `DiffTestCore` uses: `pythonRegexToCore` then `Core.verify`).
...
This is a runtime test (needs Python on PATH, plus the SMT solvers cvc5 and z3),
run from `StrataPythonTestExtra/` via `lake test`. The Strata side moved
in-process; the Python `re` oracle stays a subprocess because there is no
...
/-- Strata-side result, mirroring `DiffTestCore`'s output vocabulary. -/
...
/-- Check whether `testStr` matches `pyRegex` (in `mode`) via Strata's SMT
    backend. Lifted from `DiffTestCore.checkMatch` (without the `--log-dir`
    path, which the test does not need). -/
