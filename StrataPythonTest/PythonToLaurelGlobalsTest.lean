...
private def importModule (moduleName : String) (alias : Option String := none) : stmt SourceRange :=
  let asName := alias.map fun name => ({ val := name, ann := sr } : StrataDDM.Ann String SourceRange)
  .Import sr { val := #[.mk_alias sr { val := moduleName, ann := sr }
    { val := asName, ann := sr }], ann := sr }

private def moduleCall (moduleName methodName : String) : expr SourceRange :=
  .Call sr
    (.Attribute sr (loadName moduleName) { val := methodName, ann := sr } (.Load sr))
    { val := #[], ann := sr }
    { val := #[], ann := sr }
...
#guard
  match pythonToLaurel {}
      #[importModule "logging", assign "logger" (moduleCall "logging" "getLogger")] with
  | .ok (program, ctx) =>
    program.staticFields.isEmpty && ctx.moduleGlobals.isEmpty
  | .error _ => false

#guard
  match pythonToLaurel {}
      #[importModule "logging" (some "log"), assign "logger" (moduleCall "log" "getLogger")] with
  | .ok (program, ctx) =>
    program.staticFields.isEmpty && ctx.moduleGlobals.isEmpty
  | .error _ => false

-- Other unmodeled module calls remain modeled globals.
#guard
  match pythonToLaurel {}
      #[importModule "os", assign "value" (moduleCall "os" "getenv")] with
  | .ok (program, ctx) =>
    (program.staticFields.map (·.name.text)).contains "value" &&
    ctx.moduleGlobals.contains "value"
  | .error _ => false
...
#guard
  ...
  let notOne := expr.UnaryOp sr (.Not sr) (intLiteral 1)
...
#guard
  let aliasForms : List (stmt SourceRange × List String) := [
    (assign "T" (loadName "int"), ["T"]),
    (assignChained "T" "T2" (loadName "int"), ["T", "T2"]),
    (annAssign "T" (loadName "type") (some (loadName "int")), ["T"]),
    (annAssign "T" (loadName "TypeAlias") (some (loadName "int")), ["T"]),
    (annAssign "T" (loadName "TypeAlias") none, ["T"]),
    (assign "T" (subscriptType "list" (loadName "int")), ["T"]),
    (assign "T" (subscriptType "dict" (loadName "int")), ["T"])
  ]
  aliasForms.all fun (aliasStmt, expectedUnsupported) =>
    match pythonToLaurel {} #[aliasStmt] with
    | .ok (program, ctx) =>
      program.staticFields.isEmpty &&
      ctx.unsupportedModuleGlobals.toList.mergeSort == expectedUnsupported
    | .error _ => false
