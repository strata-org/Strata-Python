...
and that `typeTesters` is empty (declared types are caller-checked
preconditions built by `buildSpecBody`).
-/
...
/-! ## Test: buildSpecBody lowers declared types into the procedure spec

Verifies that even though `typeTesters` on `PythonFunctionDecl` is empty,
the pyspec Laurel procedure carries the type checks generated by
`buildSpecBody`: a declared parameter type becomes a caller-checked
`requires`, and the declared return type a free `ensures`. A PySpec model is
bodiless, so the spec is the only place these can live. -/

/--
info: procedure test_typed_func(x: Any, y: Any)
  returns (result: Any)
  requires Any..isfrom_int(x)( summary "declared type of parameter 'x'")
  requires Any..isfrom_str(y)( summary "declared type of parameter 'y'")
  opaque
  free ensures Any..isfrom_float(result)( summary "return type of 'test_typed_func'")
  modifies *;
-/
#guard_msgs in
#eval! do
